...
const readOne = async (db, _id) =>
  await db.collection("roles").findOne({ _id: _id });

const create = async (db, body) => {
  const inserted = await db.collection("roles").insertOne({
    ...body,
    createdAt: new Date(),
    updatedAt: new Date(),
  });
...
const update = async (db, _id, body) => {
  ...
  const doc = await db.collection("roles").findOne({ _id: _id });
  return doc;
};

const deleteOne = async (db, _id) => {
  await db.collection("roles").deleteOne({ _id: _id });
};
